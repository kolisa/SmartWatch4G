...
public sealed class Spo2QueryService : ISpo2QueryService
{
    ...
    public Spo2QueryService(ISpo2DataRepository spo2Repo, IDateTimeService dt)
    {
        _spo2Repo = spo2Repo;
        _dt = dt;
    }
    ...
    private IReadOnlyList<Spo2ReadingDto> Map(
        IEnumerable<Domain.Entities.Spo2DataRecord> records, TimeZoneInfo? tz)
        => records.Select(r => MapOne(r, tz)).ToList();

    private Spo2ReadingDto MapOne(Domain.Entities.Spo2DataRecord r, TimeZoneInfo? tz) => new()
    {
